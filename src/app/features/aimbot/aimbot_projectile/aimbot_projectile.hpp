...
private:
	...
	bool canSee(C_TFPlayer *const local, C_TFWeaponBase *const weapon, const ProjectileInfo &proj_info, Target &target, const vec3 &target_origin, const vec3 &target_pos);
	bool canSeeSplash(C_TFPlayer *const local, C_TFWeaponBase *const weapon, const ProjectileInfo &proj_info, Target &target, const vec3 &target_pos);
	bool solveTarget(C_TFPlayer *const local, C_TFWeaponBase *const weapon, const ProjectileInfo &proj_info, Target &target);
	bool getTarget(C_TFPlayer *const local, C_TFWeaponBase *const weapon, const ProjectileInfo &proj_info, Target &out);
