...
bool ProjSim::getProjInfo(C_TFPlayer *const pl, C_TFWeaponBase *const wep, const vec3 &angs, ProjectileInfo &out)
{
	if (!pl || !wep) {
		return false;
	}
	...
	switch (wep->GetWeaponID())
	{
		...
		{
			projFireSetup(pl, { 16.0f, 8.0f, -6.0f }, angs, pos, ang, true);

			const bool is_lochnload{ wep->m_iItemDefinitionIndex() == Demoman_m_TheLochnLoad };
			const float speed{ tf_utils::attribHookValue(1200.0, "mult_projectile_speed", wep) };
		...
		case TF_WEAPON_FLAREGUN: {
			projFireSetup(pl, { 23.5f, 12.0f, ducking ? 8.0f : -3.0f }, angs, pos, ang, false);
		...
		{
			projFireSetup(pl, { 23.5f, 8.0f, -3.0f }, angs, pos, ang, false);
		...
		case TF_WEAPON_SYRINGEGUN_MEDIC: {
			projFireSetup(pl, { 16.0f, 6.0f, -8.0f }, angs, pos, ang, false);
...
void ProjSim::projFireSetup(C_TFPlayer *const pl, vec3 offset, const vec3 &ang_in, vec3 &pos_out, vec3 &ang_out, bool pipes)
{
...
bool ProjSim::init(C_TFPlayer *const pl, C_TFWeaponBase *const wep, const vec3 &angs)
{
	ProjectileInfo proj_info{};

	if (!getProjInfo(pl, wep, angs, proj_info)) {
		return false;
	}
