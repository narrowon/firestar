...
constexpr float NOTIF_SPACING_X{ 20.0f };
constexpr float NOTIF_SPACING_Y{ 20.0f };
...
void Notifs::add(std::string_view str, const NotifType type)
{
	...
	float pos_y{ 50.0f + (NOTIF_SPACING_Y * m_notifs.size()) };
	...
	ImGui::PushFont(fonts::font_esp);
	m_notifs.push_back
	({
		std::string{ str },
		clr,
		i::engine->Time() + cfg::notifs_duration,
		{ (draw->getScreenSize().x - NOTIF_SPACING_X) - ImGui::CalcTextSize(str.data()).x, pos_y}
	});
	ImGui::PopFont();

	if (cfg::notifs_print_to_con) {
		i::cvar->ConsoleColorPrintf(clr, std::format("[pipeline] {}\n", str).c_str());
	}
}
...
void Notifs::run()
{
	for (size_t n{}; n < m_notifs.size(); n++)
	{
		...
		ImGui::PushFont(fonts::font_esp);
		const vec2 rect_size{ vec2(ImGui::CalcTextSize(notif.m_str.c_str()).x, ImGui::CalcTextSize(notif.m_str.c_str()).y) + vec2{ 5.0f, 6.0f } };
		ImGui::PopFont();
		...
		draw->string(fonts::font_esp, notif.m_pos, notif.m_str, notif.m_clr);
	}
}