...
bool CritsDamage::onGameEvent(IGameEvent *const event)
{
	...
	/**
	 *
	 * Use this shit for events or make a new one onGameEvent that runs with hInsts onLoad
	 * 
	 */
	const hash_t event_hash{ HASH_RT(event->GetName()) };

	if (event_hash == HASH_CT("vote_cast")) {

			/*if (!cfg::misc_vote_log)
				return true;

			if (C_TFPlayer * local{ ec->getLocal() })
			{
				player_info_t info{};
				const int vote_option = event->GetInt("vote_option");
				const int team = event->GetInt("team");

				if (int idx = event->GetInt("entityid"))
				{

					i::engine->GetPlayerInfo(idx, &info);

					std::stringstream ss;
					ss << info.name;
					ss << " voted ";
					ss << (vote_option == 1 ? "no" : "yes");
					ss << " on team ";
					ss << (team == 2 ? "RED" : "BLU");

					notifs->warning(ss.str());
				}
			}*/
	}

	if (event_hash == HASH_CT("player_connect_client")) {
		//if (!cfg::misc_logclientconnect)
		//	return true;

		//C_TFPlayer* local{ ec->getLocal() };
		//if (!ec->getLocal())
		//	return true;

		//const std::string s_id = event->GetString("networkid");
		//const std::string name = event->GetString("name");
		//const int userid = event->GetInt("userid");

		//player_info_t pi{};
		//i::engine->GetPlayerInfo(local->entindex(), &pi);
		//auto steamid = pi.guid;
		///*
		// *std::stringstream msg;
		//		msg << "dealt: " << dmg*/
		//std::stringstream msg;
		//msg << "Player " << name << " " << steamid << " has connected to the server";
		//notifs->warning(msg.str());
	}

	if (event_hash == HASH_CT("player_disconnect")) {
		//if (!cfg::misc_logclientconnect)
		//	return true;
		...
		//		msg << "dealt: " << dmg*/
		//std::stringstream msg;
		//msg << "Player " << name << " " << steamid << " has disconnected from the server";
		//notifs->warning(msg.str());
	}
	...
	if (event_hash == HASH_CT("player_hurt"))
	{
		...
		const int attacker = event->GetInt("attacker");

		player_info_t pi{};
		i::engine->GetPlayerInfo(local->entindex(), &pi);
		int steamid = pi.userID;

		// Get damage values
		const int dmg = event->GetInt("damageamount");
		int damage_amount{ event->GetInt("damageamount") };
		const int remainingHealth = event->GetInt("health");
		/*if (cfg::misc_logcritsanddamage) {
			if (steamid == attacker) {
				std::stringstream msg;
				msg << "dealt: " << dmg
					<< " | remaining health: " << remainingHealth
					<< " | crit: " << (event->GetBool("crit") ? "yes" : "no");
		...
		if (cfg::hitmarker) {
			if (steamid == attacker) {
				message_t new_msg{};
				new_msg.dmg = dmg;
				new_msg.name = event->GetString("name");
				new_msg.timer = 100;
				new_msg.wasSuccess = true;
				new_msg.index = victim->entindex();
				logging->add_msg(new_msg);
			}

			if (aimbot_hitscan->shot_at_invalid_tick) {
				message_t new_msg{};
				new_msg.name = event->GetString("name");
				new_msg.dmg = dmg;
				new_msg.timer = 200;
				new_msg.wasSuccess = false;
				new_msg.index = victim->entindex();
				new_msg.reason = "backtrack failure";
				logging->add_msg(new_msg);
			}
			if (aimbot_hitscan->missed_due_to_resolver) {
				message_t new_msg{};
				new_msg.name = event->GetString("name");
				new_msg.dmg = dmg;
				new_msg.timer = 200;
				new_msg.wasSuccess = false;
				new_msg.index = victim->entindex();
				new_msg.reason = "resolver";
				logging->add_msg(new_msg);
			}
		}*/

		
		/*if (attacker == local->entindex() && victim) {
			aimbot_hitscan->onShotFired(victim, true);
		}*/
