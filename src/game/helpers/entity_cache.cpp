...
void EntityCache::update()
{
	...
	for (int n{}; n < i::ent_list->GetHighestEntityIndex(); n++)
	{
		...
		if (class_id == class_ids::CBaseAnimating)
		{
		...
		if (class_id == class_ids::CTFAmmoPack)
		{
			C_BaseAnimating *const animating{ ent->cast<C_BaseAnimating>() };
		...
		if (class_id == class_ids::CHalloweenGiftPickup)
		{
			C_BaseAnimating *const animating{ ent->cast<C_BaseAnimating>() };
		...
		if (class_id == class_ids::CCurrencyPack)
		{
			C_CurrencyPack *const curpak{ ent->cast<C_CurrencyPack>() };

			if (!curpak || curpak->m_bDistributed()) {
				continue;
			}
		...
		if (class_id == class_ids::CTFPlayerResource) {
