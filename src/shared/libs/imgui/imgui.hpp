...
struct ImGuiIO
{
    ...
    float       MouseDoubleClickMaxDist;        // = 6.0f           // dist threshold to stay in to validate a double-click, in pixels.
    float       MouseDragThreshold;             // = 6.0f           // dist threshold before considering we are dragging.
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // dist to next character (= data from font + ImFontConfig::GlyphExtraSpacing.x baked in)
