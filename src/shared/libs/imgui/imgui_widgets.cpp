...
#include "../../../app/features/ui/ui.hpp"
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float square_sz = ImGui::GetFrameHeight() - 8;
    float spacing = style.ItemInnerSpacing.x;
    ImVec2 cursor_pos = ImGui::GetCursorScreenPos();
    ImVec2 checkbox_pos = ImVec2(cursor_pos.x + total_width - square_sz, cursor_pos.y + style.FramePadding.y);

    const ImRect total_bb(checkbox_pos, checkbox_pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.5f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImRect check_bb(checkbox_pos, checkbox_pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);
    ImColor accent_color = ImColor(100,100,100); // put this on the ui background color thing idk
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(24, 24, 24), style.FrameRounding);

    static std::map<ImGuiID, float> check_anim;
    float& check_t = check_anim[id];
    float target_check_t = 0.0f;
    if (*v)
        target_check_t = 255.f;
    else
        target_check_t = 0.f;
    ...
    //window->DrawList->AddRect(check_bb.Min - ImVec2(2.f, 2.f), check_bb.Max - ImVec2(2.f, 2.f), accent_color, style.FrameRounding);
    //window->DrawList->AddRect(check_bb.Min - ImVec2(1.f, 1.f), check_bb.Max + ImVec2(1.f, 1.f), accent_color, 0);

    window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + square_sz + 2.5, check_bb.Min.y + square_sz + 2.5), ImVec2(check_bb.Max.x - square_sz - 2.5, check_bb.Max.y - square_sz - 2.5), ImColor(30, 30, 30));
    window->DrawList->AddRect(ImVec2(check_bb.Min.x + square_sz + 1, check_bb.Min.y + square_sz + 1), ImVec2(check_bb.Max.x - square_sz - 1, check_bb.Max.y - square_sz - 1), ImColor(0, 0, 0, alpha), 0);
    //if (check_t > 0.0f)
    //{
    
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + square_sz +2.5, check_bb.Min.y + square_sz + 2.5), ImVec2(check_bb.Max.x  - square_sz - 2.5, check_bb.Max.y - square_sz - 2.5), ImColor(menu->menu_accent.Value.x, menu->menu_accent.Value.y, menu->menu_accent.Value.z, check_t), 0);
    //}
    ...
    //if (*v)
    //{
        //window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, accent_color, accent_color, border_color, border_color);
        //window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, accent_color);

    ImVec2 label_pos = ImVec2(cursor_pos.x, cursor_pos.y + style.FramePadding.y);
    if (label_size.x > 0.0f)
        window->DrawList->AddText(label_pos, ImColor(250, 250, 250, static_cast<int>(current_alpha)), label);
    //}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
       ...
       // Create a gradient-filled rectangle
        ImColor col_top(25, 25, 25);
        ImColor col_bottom(17, 17, 17);
        int steps = 12;  // The number of steps to create the gradient
        ...
        for (int i = 0; i < steps; ++i)
        {
            ...
            ImColor color_start = ImColor(
                ImLerp(col_top.Value.x, col_bottom.Value.x, t1),
                ImLerp(col_top.Value.y, col_bottom.Value.y, t1),
                ImLerp(col_top.Value.z, col_bottom.Value.z, t1),
                ImLerp(col_top.Value.w, col_bottom.Value.w, t1)
            );

            ImColor color_end = ImColor(
                ImLerp(col_top.Value.x, col_bottom.Value.x, t2),
                ImLerp(col_top.Value.y, col_bottom.Value.y, t2),
                ImLerp(col_top.Value.z, col_bottom.Value.z, t2),
                ImLerp(col_top.Value.w, col_bottom.Value.w, t2)
            );

            float y1 = ImLerp(bb.Min.y, bb.Max.y, t1);
            float y2 = ImLerp(bb.Min.y, bb.Max.y, t2);
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0, 0, 0, 100), 2, popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop : ImDrawFlags_::ImDrawFlags_RoundCornersAll);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        //  window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, ImColor(33, 33, 33), 2, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y + 3), text_col, popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.6f);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 10), window->DC.CursorPos + ImVec2(window->Size.x - 10, 20 + label_size.y + style.FramePadding.y * 1.3f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(total_bb, style.FramePadding.y);
    ...
    // Draw outline (white)
    const float outline_thickness = 1.0f; // Thickness of the outline
    ImVec2 outline_min = ImVec2(frame_bb.Min.x - outline_thickness, frame_bb.Min.y + 8 - outline_thickness);
    ImVec2 outline_max = ImVec2(frame_bb.Max.x + outline_thickness, frame_bb.Max.y - 5 - 9 + outline_thickness);
    window->DrawList->AddRect(outline_min, outline_max, ImColor(0, 0, 0, 85), g.Style.FrameRounding, ImDrawFlags_RoundCornersAll, outline_thickness);

    // Draw frame (main)
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 8), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 5 - 9), ImColor(30, 30, 30), true, g.Style.FrameRounding);
    ...
    // Render grab Slider Color
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 8), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 5 - 7), ImColor(menu->menu_accent.Value.x, menu->menu_accent.Value.y, menu->menu_accent.Value.z), style.GrabRounding);
    }
    ...
    if (g.ActiveId == id) {
        target_alpha = 250.0f; // Active (grabbed)
    }
    else if (hovered) {
        target_alpha = 140.0f; // Hovered
    }

    // Interpolate alpha towards the target
    float speed = 4.0f * ImGui::GetIO().DeltaTime; // Adjust the speed as needed
    current_alpha = ImLerp(current_alpha, target_alpha, speed);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 0 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 28), ImColor(255, 255, 255, static_cast<int>(current_alpha)), value_buf, value_buf_end); // Adjusted Y-coordinate

    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y - 28), ImColor(255, 255, 255, static_cast<int>(current_alpha)), label);
